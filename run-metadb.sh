...
echo "Testing if $DATA_DIR exists"

# Check if DATA_DIR exists, if not then runs metadb init
INIT_FLAG="false"
if [ ! -d "$DATA_DIR" ]; then
  echo "$DATA_DIR does NOT exist, starting initialization process"
  mkdir -p "$DATA_DIR"
  rm -r "$DATA_DIR"
  /usr/bin/metadb init -D "$DATA_DIR"
...
if [ ! -f "$LOG_FILE_PATH" ]; then
  ...
  echo "Created Log File at $LOG_FILE_PATH"
fi

# Generate metadb.conf
if [ -f "$DATA_DIR/metadb.conf" ]; then
  rm -f "$DATA_DIR/metadb.conf"
  echo 'Deleting metadb.conf file'
fi

echo 'Generating new metadb.conf file'
...
if [ "$INIT_FLAG" = "true" ]; then
  echo 'Continuing initialization process'
  ...
  echo 'Running initial synchronization with Kafka Connect sensor (this may take awhile). Once the sync is complete ("source snapshot complete (deadline exceeded)" will appear in the log file), run MetaDB with METADB_RUN_MODE set to "endsync".'
  sleep 9999999999
fi

# Run MetaDB
if [ "$METADB_RUN_MODE" = "upgrade" && "$INIT_FLAG" = "false"]; then
  echo 'Starting MetaDB Upgrade Task (this may take awhile)'
  sudo -u metadb /usr/bin/metadb upgrade -D "$DATA_DIR" -l "$LOG_FILE_PATH"
  echo 'MetaDB Upgrade Complete! Please Change the METADB_RUN_MODE variable to "start" and restart the container.'
...
if [ "$METADB_RUN_MODE" = "sync" ]; then
  echo 'Starting MetaDB Sync Task (source: sensor)'
  sudo -u metadb /usr/bin/metadb sync -D "$DATA_DIR" --source sensor -l "$LOG_FILE_PATH"
  echo 'MetaDB Sync Complete! Please Change the METADB_RUN_MODE variable to "start" and restart the container.'
  sleep 99999999999
fi

if [ "$METADB_RUN_MODE" = "endsync" ]; then
  echo 'Starting MetaDB Endsync Task (source: sensor)'
  sudo -u metadb /usr/bin/metadb endsync -D "$DATA_DIR" --source sensor -l "$LOG_FILE_PATH"
  echo 'MetaDB Endsync Complete! Please Change the METADB_RUN_MODE variable to "start" and restart the container.'
  sleep 99999999999
fi

if [ "$METADB_RUN_MODE" = "start" ]; then
  echo 'Starting MetaDB Instance'
  if [ "$VERBOSE_LOGGING" = "true" ]; then
    sudo -u metadb /usr/bin/metadb start -D "$DATA_DIR" -l "$LOG_FILE_PATH" --port $METADB_PORT --debug --memlimit $MEM_LIMIT_GB
  fi
  if [ "$VERBOSE_LOGGING" = "false" ]; then
